  ...
  switch (req.method) {
    case 'GET':
      ...
      Maid.findAll({
        ...
        res.end(pug.renderFile('./views/maid-posts.pug', {
          posts: posts,
...
function createValidHash(originalId, userName, secretKey) {
  const sha1sum = crypto.createHash('sha1');
  sha1sum.update(originalId + userName + secretKey);
  return sha1sum.digest('hex');
}

function handleMaidRedirectPosts(req, res) {
  res.writeHead(303, {
    'Location': '/posts'
  });
  res.end();
